...
def load_models():
    ...
    # Try multiple possible paths for the custom model
    possible_paths = [
        os.path.join(os.path.dirname(__file__), 'Model', 'weights', 'best.pt'),
    ...
    if model is None:
        st.error("Could not find or load the custom trained model (best.pt)")
        st.info("Using base YOLO model only - PPE detection may be limited")
        model = model1
...
def run_webcam_detection(pose, model):
    ...
    if os.environ.get('STREAMLIT_CLOUD_ENVIRONMENT'):
        st.warning("⚠️ Webcam functionality is not available on Streamlit Cloud.")
        st.info("Please use the 'Upload Video' option to analyze video files instead.")
        return
    ...
    if not cap.isOpened():
        ...
        st.info("• No webcam is connected")
        st.info("• Webcam is being used by another application")
        st.info("• Running on a platform without webcam access (like Streamlit Cloud)")
        st.info("Try using the 'Upload Video' option instead.")
        return
